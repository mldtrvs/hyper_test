...
class docDetailsHelper:
    ...
    def edit(self, wait, edit):
        ...
        edit_btn = wait.until(EC.element_to_be_clickable((
            By.CSS_SELECTOR, "#grid-308_tab [role=toolbar] [buttonrole=edit]")))
        edit_btn.click()
        detail_name_edit = self.app.driver.find_element(
            By.CSS_SELECTOR, f"#form-309-{record_id_number}_popup [name=type_name]")
        detail_name_edit.clear()
        detail_name_edit.send_keys(edit)

        ok_btn = wait.until(
            EC.element_to_be_clickable((By.ID, f"form-309-{record_id_number}_popup_save-button")))
        ok_btn.click()
