...
class FilmTypeHelper:
    ...
    def add_new(self, wait, film_type):
        add_btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#grid-331_tab [role=toolbar] ["
                                                                          "buttonrole=add]")))
        add_btn.click()
        self.app.driver.find_element(By.CSS_SELECTOR, "#form-332--1_popup [name='type_name']").send_keys(film_type)
    ...
    def check_if_added(self):
        total_records = self.app.driver.find_element(By.ID, "grid-331_tab_totalCount")
    ...
    def edit(self, wait, edit):
        ...
        edit_btn = wait.until(EC.element_to_be_clickable((
            By.CSS_SELECTOR, "#grid-331_tab [role=toolbar] [buttonrole=edit]")))
        edit_btn.click()
        film_type_edit = self.app.driver.find_element(
            By.CSS_SELECTOR, f"#form-332-{record_id_number}_popup [name='type_name']")
        film_type_edit.clear()
        film_type_edit.send_keys(edit)

        ok_btn = wait.until(
            EC.element_to_be_clickable((By.ID, f"form-332-{record_id_number}_popup_save-button")))
        ok_btn.click()

    def get_record_id(self):
        record_id = self.app.driver.find_element(By.ID, "grid-331_tab_totalId")
        text = record_id.text
