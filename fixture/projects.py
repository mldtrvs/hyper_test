...
class projectHelper:
    ...
    def random_choice_genre_type(self, wait):
        ...
        # Exclude genres containing a comma
        filtered_genres = [genre for genre in genre_types_text if ',' not in genre]

        # Randomly select the required number of genres
        random_genres = random.sample(filtered_genres, num_genre_types_to_select)

        # Print the randomly selected checkboxes
        print("Randomly selected genre_types:")
        for selected_genre in random_genres:
        ...
        for i, selected_genre in enumerate(random_genres):
        ...
        return gt_aria_owns_value, random_genres
    ...
    def get_genre_value(self, wait):
        ...
        # Execute JavaScript to get the text of the TR element
        project_tr_text_out = self.app.driver.execute_script(
            f"return document.querySelector('#grid-276_tab [data-id=\"{record_id_number}\"]').textContent"
        )

        print(project_tr_text_out)
        return project_tr_text_out

    def check_selected_genres_match(self, selected_genre, project_tr_text_out):
        # Extract the list of genres from the selected_genre tuple
        _, genre_list = selected_genre

        # Check if all genres in the list are in the project_tr_text_out
        for genre in genre_list:
            if genre not in project_tr_text_out:
                raise AssertionError(
                    f"Tag type values differ. Expected all of: {genre_list}, Actual: {project_tr_text_out}")
        # If all genres are found, it means they match
        print("Genre types match")
